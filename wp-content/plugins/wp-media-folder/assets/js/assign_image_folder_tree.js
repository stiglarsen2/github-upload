    ...
    wpmfAssignModule = {
        ...
        initModule: function ($current_frame) {
            ...
            if (!$current_frame.find('.open-popup-tree-multiple').length) {
                $current_frame.find('.media-frame-content .media-toolbar-secondary .delete-selected-button').after('<button class="button open-popup-tree-multiple media-button button-large"><i class="zmdi zmdi-folder"></i>' + wpmf.l18n.assign_tree_label + '</button>');
        ...
        openfolderassign: function (id) {
            ...
            ret = $.ajax({
                ...
                this.find('a[data-="' + id + '"]').closest('li').removeClass('wait').removeClass('collapsed').addClass('expanded');
