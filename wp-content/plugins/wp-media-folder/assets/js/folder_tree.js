...
(function ($) {
    ...
    wpmfFoldersTreeModule = {
        ...
        getRendering: function getRendering() {
            var ij = 0;
            var content = ''; // Final tree view cwpmf-folder-tree-resizeontent
            ...
            var generateList = function generateList() {
                ...
                while (ij < wpmfFoldersTreeModule.categories.length) {
                    ...
                    var icondrive = '<i class="material-icons" style="' + bgcolor + '">folder</i>';
        ...
        initContainerResizing: function initContainerResizing($current_frame) {
            ...
            if (wpmf.vars.wpmf_pagenow === 'upload.php' && wpmfFoldersModule.page_type) {
                ...
                var hideMenuTreeSize = wpmfFoldersModule.getCookie('hide-menu-tree-size');
                if (typeof hideMenuTreeSize !== "undefined" && parseFloat(hideMenuTreeSize) > 0) {
                    // Set positioning of the different elements
                    _$tree.css('width', parseFloat(hideMenuTreeSize) + 'px');
                    _$handle.css('left', parseFloat(hideMenuTreeSize) + 10 + 'px');
                    _$attachments.css('left', parseFloat(hideMenuTreeSize) + 4 + 'px');
                    $('.wpmf-folder-tree, .wpmf-folder-tree > ul').css('max-width', '100%');
                } else {
                    // Set handle initial position
                    setTimeout(function () {
                        _$tree.css('width', 'calc(20% + 10px)');
                        _$handle.css({ right: 'auto', left: 'calc(20% + 10px)' });
                    }, 500);
                }
                ...
                var menuTreeSize = wpmfFoldersModule.getCookie('menu-tree-size');
