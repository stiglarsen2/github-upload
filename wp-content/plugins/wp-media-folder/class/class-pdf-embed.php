...
class WpmfPdfEmbed
{
    ...
    public function wpmfPdf($attrs)
    {
        ...
        if (!empty($attrs['embed'])) {
            ...
            $return = '<a data-wpmf_pdf_embed="link" href="'.esc_url($url).'" target="'.esc_attr($attrs['target']).'">'.esc_html($pdf->post_title).'</a>';
        }
    ...
    public function loadStyleScript()
    {
        global $post;
        if (!empty($post)) {
            if (strpos($post->post_content, 'data-wpmf_pdf_embed="embed"') || strpos($post->post_content, '[wpmfpdf')) {
                wp_enqueue_script(
                    'wpmf_embed_pdf_js',
