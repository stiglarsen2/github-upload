...
class WpmfDisplayGallery
{
    ...
    public function localizeScript()
    {
        ...
        return array(
            ...
            'smush_lazyload' => (isset($smush_settings) && isset($smush_settings['lazy_load']) && $smush_settings['lazy_load']) ? true : false
        );
    }
    ...
    public function getAttachmentLink(
        ...
        return apply_filters(
            ...
            '<a class="' . $class . '" data-lightbox="' . $lb . '" href="' . $url . '" data-title="' . esc_attr($title) . '" title="' . esc_attr($_post->post_title) . '" target="' . $target . '">' . $text . '</a>',
            $id,
            $size,
            $permalink,
            false,
