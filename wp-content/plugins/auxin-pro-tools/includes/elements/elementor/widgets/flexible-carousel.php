    ...
    protected function render() {
		...
		if ( ! empty( $slides ) ) {
			...
			foreach (  $slides as $slide ) {
                $template = get_page_by_path( $slide['template'], OBJECT, 'elementor_library' );
                if( isset( $template->ID ) && $template->ID !== ' ' && get_post_status( $template->ID ) ){
                    echo sprintf( '<div %s>%s</div>', $this->get_render_attribute_string( 'item_attr' ), Plugin::instance()->frontend->get_builder_content_for_display( $template->ID ) );
                }
            }
